...
function ShowTimeDetail({ open, onClose, showTimeId }) {
  ...
  // Disable edit if startTime < now (compare both date and time)
  let isPast = false;
  if (showTime && showTime.data && showTime.data.startTime && showTime.data.date) {
    // Đảm bảo format ngày luôn là YYYY-MM-DD
    const dateStr = dayjs(showTime.data.date).format('YYYY-MM-DD');
    const showStart = dayjs(`${dateStr} ${showTime.data.startTime}`, 'YYYY-MM-DD HH:mm:ss');
    isPast = showStart.isValid() ? showStart.isBefore(dayjs()) : false;
  }
          ...
          {showTime && showTime.data ? (
            <>
              <Descriptions column={1} bordered>
                ...
                <Descriptions.Item label="Date">{showTime.data.date ? dayjs(showTime.data.date).format('YYYY-MM-DD') : ''}</Descriptions.Item>
